...
export default function BillingTab({
  ...
  // Usage stats (approximate from available props)
  const usage = useMemo(() => {
    const subCount = subscribedRoles?.length || 0;
    return [
      { name: "Labs",              cur: isPaid ? subCount : Math.min(subCount, 1), max: isPaid ? Math.max(12, subCount) : 2, suffix: "used" },
      { name: "Interview Attempts", cur: isPaid ? 0 : 0,                            max: isPaid ? Math.max(20, subCount * 5) : 2, suffix: "used" },
      { name: "Path Access",        cur: isPaid ? 1 : 0,                            max: 1,                                        suffix: "level", textLabel: isPaid ? "Premium" : "Basic" },
    ];
  }, [subscribedRoles, isPaid]);
  ...
  return (
    <>
      ...
      <div className="tr-bill-root" style={{ opacity: !user ? 0.4 : 1, pointerEvents: !user ? "none" : "auto" }}>
        ...
        {/* ── Paired grid: each row = one subscription card + one account card ── */}
        <div className="tr-bill-layout">

          {/* Row 1 LEFT: Current Plan */}
          ...
          {/* Row 2 LEFT: Purchased Course Details */}
          <div className="tr-bill-card fadeUp">
