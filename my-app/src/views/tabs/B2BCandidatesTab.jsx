...
export default function B2BCandidatesTab({
  ...
  // ═══════════════════════════════════════════════════════════════
  // RICH REPORT HTML GENERATOR (matches the email report)
  // Used by View, Download, and Bulk Download
  // ═══════════════════════════════════════════════════════════════
  const generateReportHTML = (cand, autoPrint = false) => {
    ...
    const evalRows = evals.map((e, i) => {
      ...
      return `<div class="q-block ${cls}">
        <div class="q-header">
          <span class="q-num">QUESTION ${i + 1} · ${escape(e.category || 'General')}</span>
    ...
    return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Report — ${escape(cand.name)}</title>
<style>
...
<body>
  ...
  <div class="score-card">
    ...
    <div class="score-meta">out of 10 · ${escape(roleName)} · ${escape(cand.difficulty || '')}</div>
  ...
  ${topStrength ? `<div class="section s2">
    ...
    <div class="sec-meta">${escape(topStrength.category || 'General')} — Q${evals.indexOf(topStrength) + 1} (${topStrength.score}/10)</div>
  ...
  ${topWeakness ? `<div class="section s3">
    ...
    <div class="sec-meta">${escape(topWeakness.category || 'General')} — Q${evals.indexOf(topWeakness) + 1} (${topWeakness.score}/10)</div>
    <div class="sec-text">${escape(topWeakness.weaknesses || 'None')}</div>
  </div>` : ''}

  <div class="section s4">
    <div class="sec-title">4. Recommended Next Steps</div>
    ${nextSteps.map((s, i) => `<div class="step"><span class="step-num">${i + 1}.</span><span class="step-txt">${escape(s)}</span></div>`).join('')}
  </div>
  ...
  <div class="footer">Assessment completed on ${cand.completed_at?.substring(0, 10) || new Date().toLocaleDateString()} · ThreatReady Cybersecurity Platform</div>
